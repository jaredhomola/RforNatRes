...
Question 2e. Add two more line of code to determine whether the mean ice_duration was longer in years that had ice on occur in November, December, or January.       
```{r}
# Answer 2e
...
The next set of data we'll analyze is the Lahman::Salaries data, which includes salary data for all Major League Baseball players since 1985. This question will have just one part, but it will include a pipeline that strings multiple Tidyverse functions together to generate the answer.   
...
Modify the R Markdown document as needed to ensure the only output that is displayed is 1) the questions, 2) your code and 3) the resulting tibbles for each question/sub-question.  